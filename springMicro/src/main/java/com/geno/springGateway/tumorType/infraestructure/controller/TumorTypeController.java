...
public class TumorTypeController {
    ...
    @ApiResponses({
            @ApiRestTemplateResponse(responseCode = "200", description = "Tipo de tumor encontrado con éxito."),
            @ApiRestTemplateResponse(responseCode = "404", description = "Tipo de tumor no encontrado (ID no existe).")
    })
    @GetMapping("/{id}")
    ...
    @ApiResponses({
            @ApiRestTemplateResponse(responseCode = "201", description = "Tipo de tumor creado exitosamente."),
            @ApiRestTemplateResponse(responseCode = "400", description = "Solicitud inválida (error de validación en los datos de entrada).")
    })
    @PostMapping
    public ResponseEntity<ApiRestTemplateResponse<TumorTypeOutDto>> create(@RequestBody TumorTypeInDto dto) {
        TumorTypeOutDto created = tumorTypeService.create(dto);
        return new ResponseEntity<>(new ApiRestTemplateResponse<>("Created", "Tipo de tumor creado", created), HttpStatus.CREATED);
    }


    @Operation(summary = "Actualizar Nombre (Parcial)", description = "Actualiza solo el campo 'name' del Tipo de Tumor.")
    @ApiResponses({
            @ApiRestTemplateResponse(responseCode = "200", description = "Nombre de tipo de tumor actualizado con éxito."),
            @ApiRestTemplateResponse(responseCode = "400", description = "Solicitud inválida (el nombre está vacío o es nulo)."),
            @ApiRestTemplateResponse(responseCode = "404", description = "Tipo de tumor no encontrado (ID no existe).")
    })
    @PatchMapping("/{id}/name")
    public ResponseEntity<ApiRestTemplateResponse<TumorTypeOutDto>> patchName(
    ...
    @ApiResponses({
            @ApiRestTemplateResponse(responseCode = "200", description = "Sistema afectado actualizado con éxito."),
    ...
    @ApiResponses({
            @ApiRestTemplateResponse(responseCode = "200", description = "Tipo de tumor eliminado con éxito."),
            @ApiRestTemplateResponse(responseCode = "404", description = "Tipo de tumor no encontrado (ID no existe).")
    })
    @DeleteMapping("/{id}")
