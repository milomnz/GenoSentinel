...
export class ClinicalRecordController{
    ...
    @ApiResponse({ status: 201, description: 'Registro clinico creado exitosamente', type: ClinicalRecord })
    @ApiResponse({
        status:404,
    ...
    @ApiOperation({ summary:'Elimina un registro clinico', description: 'Elimina permanentemente un registro clinico por su ID' })

    @ApiParam({ name:'id', description:'ID numerico del registro clinico a eliminar', type:String })
    @ApiResponse({ status: 204, description:'Registro clinico eliminado exitosamente'})
    @ApiResponse({ status: 404, description: 'Registro clinico no encontrado' })
    @HttpCode(HttpStatus.NO_CONTENT)
    @Delete(':id')
    delete(@Param('id', ParseIntPipe) id: number): Promise<void>{
        return this.clinicalRecordService.delete(id);
    }
    @ApiOperation({ summary: 'Actualiza parcialmente un registro clinico', description: 'Permite actualizar uno o mas campos de un registro clinico existente sin afectar los demas campos'})
    @ApiResponse({ status: 200, description: 'Registro clinico actualizado exitosamente', type: ClinicalRecord })
    @ApiResponse({ status: 404, description: 'Registro clinico no encontrado' })
    @Patch('update/:id')
